...
/**
 * スマホ版スクリーン。上から「敵とミッション → 問題 → エティモン管理」の順。
 ...
 * - 最下段はエティモン管理（ボックス管理画面の上部ヘッダののぞき見）。
 *   タップ頻度・表示の変化がもっとも少ないため画面端に置く。ヘッダを
 *   上へ引くと同じ盤面が地続きにせり上がり全画面のボックス管理になる
  ...
  return (
    <div className="quiz-shell fantasy-shell relative h-dvh overflow-hidden">
      ...
      <div className="relative z-10 flex h-full w-full flex-col gap-2 p-2">
        ...
        {/* ── 3. エティモン管理（最下段に固定配置。手持ち欄は廃し、
             ボックス管理画面の上部ヘッダをそのままここへのぞかせる。
             つまみ／ヘッダを上へ引くと同じ盤面が下から地続きにせり上がる。
             背景・盤面は fixed で全画面に展開するため、この footer は
             のぞかせるヘッダぶんの高さ（PEEK_PX と一致）を確保する枠） ── */}
