  ...
  return (
    ...
    >
      <div className="min-h-0 flex-1 overflow-y-auto px-4 pt-3 sm:px-6">
        <div className="rounded-lg border-2 border-[#3aa83a] bg-[#0e120b] p-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-baseline gap-2">
              <motion.span
                ...
                className="text-3xl font-bold text-[#61ff5f] drop-shadow-[0_0_10px_rgba(97,255,95,0.5)]"
              >
                {grade}
              </motion.span>
              <span className="text-sm font-medium text-[#cfeecb]">{title}</span>
            </div>
            <div className="text-sm tabular-nums text-[#6f9268]">
              {score} / {playLimit} 正解
            </div>
          </div>

          <div className="relative mt-2 overflow-hidden rounded-md border border-[#3aa83a]/50 bg-[#060803] px-3 py-2 text-center">
            <div className="relative flex items-baseline justify-center gap-1.5">
              <span className="text-3xl font-bold tabular-nums text-[#61ff5f]">
                {(xp ?? 0).toLocaleString()}
              </span>
          ...
          {message && (
            <p className="mt-2 text-xs leading-relaxed text-[#cfeecb]">{message}</p>
          )}
          ...
          <div className="mt-2 grid gap-1.5 sm:grid-cols-2">
            {breakdown?.map((item) => (
              <div
                key={item.label}
                className="rounded-md border border-[#3aa83a]/25 bg-[#060803] px-2.5 py-1.5 text-xs text-[#cfeecb]"
              >
