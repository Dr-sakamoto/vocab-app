...
interface StarterChoiceModalProps {
  onSelect: (lineId: string) => void;
}

export default function StarterChoiceModal({ onSelect }: StarterChoiceModalProps) {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="fixed inset-0 z-[70] flex items-center justify-center bg-zinc-950/60 p-4"
    >
      <motion.div
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          {choices.map((choice, i) => (
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4 text-center transition-colors hover:border-emerald-300 hover:bg-emerald-50 hover:shadow-lg hover:shadow-emerald-200/50"
            >
