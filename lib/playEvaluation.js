...
// ── フローゾーン係数 ───────────────────────────────────────────────────────
// Csikszentmihalyi(1990): 60-80%正解帯が最も深い符号化を促すフローゾーン
function flowZoneMultiplier(accuracy) {
  if (accuracy >= 0.60 && accuracy <= 0.80) return 1.4;
  if (accuracy > 0.80  && accuracy < 1.00)  return 1.2;
  if (accuracy === 1.00)                     return 1.1;
  if (accuracy >= 0.40)                      return 1.0;
  return 0.85;
}
...
// ── XP計算コア（evaluatePlay と computeSessionXP で共有） ────────────────
function computeAllXP({ answers, score, playLimit, unlockedPoolSize }) {
 ...
 */
export function computeSessionXP({ answers, score, unlockedPoolSize, playLimit }) {
  const { totalXP, tier } = computeAllXP({ answers, score, playLimit, unlockedPoolSize });
  // 後方互換のため streakXP: 0 を返す（page.js 側で使われていないが念のため）
...
export function evaluatePlay({ answers, score, playLimit, bestStreak, unlockedPoolSize }) {
  ...
  const { baseXP, weakness, newWord, netRise, totalXP, tier, fzm, multiplier } =
    computeAllXP({ answers, score, playLimit, unlockedPoolSize });
