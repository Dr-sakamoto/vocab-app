...
export function evaluatePlay({
  ...
  const bonusPoints = rawScore * (poolMultiplier - 1);
  const totalScore = Math.round(clamp(rawScore + bonusPoints, 0, 140));
  const maxScore = Math.round(100 * poolMultiplier);
  ...
  return {
    ...
    breakdown: [
      ...
      {
        label: "プール補正ボーナス",
        points: Math.round(bonusPoints),
        max: Math.round(100 * (poolMultiplier - 1)),
        detail: `池サイズ ${unlockedPoolSize}`,
      },
    ],
  };
}
