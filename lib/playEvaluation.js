...
const PERFECT_GRADE = GRADES[0];
const SCORE_GRADES = GRADES.slice(1);
...
export function evaluatePlay({ answers, score, playLimit, bestStreak, unlockedPoolSize }) {
  ...
  const gradeEntry = score === playLimit && playLimit > 0
    ? PERFECT_GRADE
    : SCORE_GRADES.find(g => accuracy >= g.minAccuracy) ?? SCORE_GRADES.at(-1);
