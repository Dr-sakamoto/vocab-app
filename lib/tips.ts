...
/**
 * 全エリア共通の攻略Tips。
 * 扱うのは、画面のラベルやミッション名からは読み取れない「気づきにくい仕様」だけ:
 * ラベルの無い隠れた操作（札をタップして進捗表示、取っ手をドラッグして編成）や、
 * 画面の数値に表れない挙動（手持ちの強さと与ダメージ・HPの減り方の関係）など、
 * 見落としがちな仕様の周知と、分かりにくい導線の補いに徹する。
 * ミッション名を言い換えただけの説明（例:「新しい単語に○回正解」＝未挑戦の単語に
 * 正解）は、プレイ中に自然と分かるため載せない。
 * 率直でわかりやすい言い方にし、遠回しな比喩は避ける。
 */
export const SYSTEM_TIPS: readonly string[] = [
  "正解するたびに相手のHPが減っていく。積まれたミッションを全部達成すれば捕獲できる。",
  "HPが0になる前にミッションを達成できないと、捕まえられずに逃げられてしまう。",
  "現在地の札をタップすると、これまで集めた単語の数と今のランクを確認できる。",
  "手持ちの上ふちの取っ手は、タップか上ドラッグで編成画面が開く。指定エティモンやレベルのミッションは、ここで手持ちを組み替えて狙う。",
