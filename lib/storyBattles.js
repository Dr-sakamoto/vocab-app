...
export function getOpponentPokemonIndex(battle, questionNumber, playLimit = getBattlePlayLimit(battle), currentAccuracy = 0) {
  ...
  // ポケモンのインデックスに応じた必要精度を計算
  // 例：3体いる場合
  //   1体目: winAccuracy * 0.6  (必要精度の60%)
  //   2体目: winAccuracy * 0.75 (必要精度の75%)
  //   3体目: winAccuracy * 0.9  (必要精度の90%)
  let currentIndex = 0;
  for (let i = 0; i < party.length - 1; i++) {
    const requiredAccuracy = winAccuracy * (0.6 + (i / (party.length - 1)) * 0.3);
...
export function getOpponentPokemonStatus(battle, questionNumber, playLimit, currentAccuracy = 0) {
  ...
  return party.map((entry, index) => {
    const requiredAccuracy = winAccuracy * (0.6 + (index / Math.max(1, party.length - 1)) * 0.3);
    const isDefeated = currentAccuracy >= requiredAccuracy;
