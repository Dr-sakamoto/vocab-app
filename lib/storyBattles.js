...
export const STORY_BATTLES = [
  {
    ...
    habitatId: "route-1",
    party: [
      { lineId: "charmander", level: 5 },
      { lineId: "squirtle", level: 7 },
      { lineId: "bulbasaur", level: 9 },
    ],
  ...
  },
  {
    ...
    rewards: { badge: "storm", unlocksSideAreas: true },
  },
  {
    id: "voltorb-trap",
    name: "ビリリダマ",
...
export function isBattleWon(score, playLimit, battle) {
  ...
  return score / playLimit >= getBattleWinAccuracy(battle);
}
...
export function setPendingChallenge(progress, battleId) {
  const normalized = normalizeStoryProgress(progress);
  if (!BATTLE_BY_ID.has(battleId)) return normalized;
  return { ...normalized, pendingChallengeId: battleId };
}

export function clearPendingChallenge(progress) {
  const normalized = normalizeStoryProgress(progress);
  return { ...normalized, pendingChallengeId: null };
}

export function startBattleSession(progress, battleId) {
  const normalized = normalizeStoryProgress(progress);
  if (!BATTLE_BY_ID.has(battleId)) return normalized;
  return {
    ...normalized,
...
export function getResumeBattle(progress) {
  ...
  return getBattleById(normalized.activeBattleId);
}
...
export function getOpponentPokemonIndex(battle, questionNumber, playLimit = getBattlePlayLimit(battle)) {
  const party = battle?.party ?? [];
  if (party.length === 0) return 0;
  const perMon = Math.max(1, Math.ceil(playLimit / party.length));
  return Math.min(party.length - 1, Math.floor(Math.max(0, questionNumber - 1) / perMon));
}

export function getOpponentPokemon(battle, questionNumber, playLimit) {
  const party = battle?.party ?? [];
  const index = getOpponentPokemonIndex(battle, questionNumber, playLimit);
...
export function syncRetroactiveBattles(progress, { unlockedPoolSize = 0 } = {}) {
  ...
  if (blocker) {
    return setPendingChallenge(normalized, blocker.id);
  }
  ...
  for (const battle of STORY_BATTLES) {
    ...
    return setPendingChallenge(normalized, battle.id);
  }
