export const GAME = {
  PLAY_LIMIT: 10,
  /**
   * 一度に画面へ出す設問の数。10問は一枚の答案として続くが、見えているのは
   * 常にこの数だけで、打ち終えるとページごと入れ替わる。
   *
   * 10問を縦に並べるとスマホではスクロールが要る。打ちながら紙が動くと
   * 視線が毎問リセットされ、次の設問を探す時間がコアループに積まれる。
   * 2問なら、キーボードが出ている状態でも1画面に収まって紙が動かない。
   *
   * 2問より少なくはしない。1問だけを出すと、確定した瞬間に画面から
   * 打つものが消え、採点の往復が「次を打つ時間」と重ならなくなる
   * （＝1問1答へ戻る）。2問あれば、1問目の採点は2問目を打つあいだに
   * 終わり、ページの切り替わりも2問に1回で済む。
   */
  QUIZ_PAGE_SIZE: 2,
