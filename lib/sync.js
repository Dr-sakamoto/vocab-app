...
import { mergeStoryProgress } from "./storyBattles.js";
...
function mergeMonsterCollections(localCollection, remoteCollection, legacyXP = 0) {
  ...
  const habitatVisits = { ...remote.habitatVisits };
  ...
  const professorTransfers = { ...remote.professorTransfers };
...
export async function uploadProgress({ stats, unlockedPoolSize, monsterCollection, approvedAnswers }) {
...
export async function downloadAndMerge({ stats, unlockedPoolSize, monsterCollection, approvedAnswers }) {
  const user = await requireSignedInUser();
  ...
  let { data: remoteMeta, error: metaError } = await supabase
    .from("user_meta")
    .select("unlocked_pool_size, monster_total_xp, active_monster_id, monster_collection, professor_transfers, approved_answers")
    .eq("user_id", user.id)
    .maybeSingle();
  ...
  // approved_answers: wordId ごとに配列の union
  const remoteApproved = remoteMeta?.approved_answers ?? {};
  const localApproved = approvedAnswers ?? {};
  const mergedApprovedAnswers = { ...remoteApproved };
  for (const [wordId, answers] of Object.entries(localApproved)) {
    const existing = new Set(mergedApprovedAnswers[wordId] ?? []);
    for (const ans of answers) existing.add(ans);
    mergedApprovedAnswers[wordId] = [...existing];
  }
  ...
  return {
    ...
    approvedAnswers: mergedApprovedAnswers,
  };
}
