...
export const DEFAULT_MONSTER_COLLECTION = {
  ...
  partyIds: ["starter-charmander", "starter-squirtle", "starter-bulbasaur"],
  monsters: [
...
export function normalizeMonsterCollection(collection, legacy = {}) {
  ...
  if (!partyIds.includes(activeId)) {
    if (partyIds.length >= PARTY_SIZE) partyIds[0] = activeId;
    else partyIds.unshift(activeId);
  }

  for (const monster of monsters) {
    if (partyIds.length >= Math.min(PARTY_SIZE, monsters.length)) break;
    if (!partyIds.includes(monster.id)) partyIds.push(monster.id);
  }
  ...
  return { version: 1, activeId: partyIds[0] ?? activeId, partyIds, monsters };
}
...
export function setActiveMonster(collection, monsterId) {
  ...
  if (currentIndex > 0) {
    ...
    if (partyIds.length >= PARTY_SIZE) partyIds[0] = monsterId;
    else partyIds.unshift(monsterId);
  }
