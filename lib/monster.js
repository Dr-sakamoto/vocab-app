...
export function getLineIdBySpeciesId(speciesId) {
    for (const line of MONSTER_LINES) {
        if (line.species.some(s => s.id === speciesId))
            return line.id;
    }
...
const SPRITE_ROOT = "https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/versions/generation-ii/";
...
function resolveGiftMinPool(event, habitatMinPools = {}) {
    if (Number.isFinite(Number(event.minPool)))
        return Number(event.minPool);
...
export function createMonsterInstance({ id, lineId = DEFAULT_MONSTER_LINE_ID, totalXP = 0, acquiredAt = null, heldItemType = null, heldItemName = null, forcedSpeciesId = null, } = {}) {
...
export function normalizeMonsterCollection(collection, legacy = {}) {
    ...
    const fallbackActiveId = monsters.find(monster => monster.lineId === legacyLineId)?.id ??
        monsters[0]?.id ??
        null;
    const activeId = requestedActiveId && monsters.some(monster => monster.id === requestedActiveId)
        ? requestedActiveId
        : fallbackActiveId;
    ...
    const storyProgress = collection?.storyProgress && typeof collection.storyProgress === "object"
...
export function getActiveMonster(collection) {
    ...
    return (normalized.monsters.find(monster => monster.id === normalized.activeId) ??
...
export function setActiveMonster(collection, monsterId) {
    const normalized = normalizeMonsterCollection(collection);
    if (!normalized.monsters.some(monster => monster.id === monsterId))
        return normalized;
...
export function sortBoxMonsters(collection, mode) {
    ...
    const sortedBoxMonsters = normalized.monsters
        ...
        if (mode === "dex") {
            return (aState.species.id - bState.species.id ||
    ...
    return normalizeMonsterCollection({
        ...normalized,
        monsters: normalized.monsters.map(monster => partyIdSet.has(monster.id) ? monster : boxQueue.shift()),
    });
}
...
export function sendPartySlotToBox(collection, partyIndex) {
    ...
    if (!Number.isInteger(partyIndex) || partyIndex < 0 || partyIndex >= PARTY_SIZE)
        return normalized;
    if (!normalized.partyIds[partyIndex])
        return normalized;
...
export function swapMonsterLocations(collection, first, second) {
    ...
    const getLocationId = (location) => {
        if (location.area === "party")
            return partyIds[location.index] ?? null;
    ...
    if (first.area !== "party" && !firstId)
        return normalized;
    if (second.area !== "party" && !secondId)
        return normalized;
...
export function sendMonstersToProfessor(collection, monsterIds = []) {
    ...
    const transferable = normalized.monsters.filter(monster => requestedIds.has(monster.id) && !partyIdSet.has(monster.id));
...
export function getTotalProfessorTransfers(collection) {
    const normalized = normalizeMonsterCollection(collection);
    return Object.values(normalized.professorTransfers ?? {}).reduce((sum, count) => sum + Math.max(0, Number(count) || 0), 0);
}
export function getPendingFossilGift(collection, { unlockedPoolSize = 0, habitatMinPools = {} } = {}) {
    ...
    if (poolSize < resolveGiftMinPool(FOSSIL_GIFT_GROUP, habitatMinPools))
        return null;
    if (isFossilGiftClaimed(normalized.giftClaims))
        return null;
...
export function claimFossilGift(collection, lineId, { habitatMinPools = {} } = {}) {
    ...
    if (!choice)
        return { collection: normalized, awarded: null };
    if (isFossilGiftClaimed(normalized.giftClaims))
        return { collection: normalized, awarded: null };
...
export function awardEligibleGiftMonsters(collection, { unlockedPoolSize = 0, trigger = "all", habitatMinPools = {} } = {}) {
    const normalized = normalizeMonsterCollection(collection);
    ...
    for (const event of GIFT_MONSTER_EVENTS) {
        if (trigger !== "all" && event.trigger !== trigger)
            continue;
        if (nextGiftClaims[event.id])
            continue;
        if (event.trigger === "professor-transfer" && totalTransfers < (event.requiredTransfers ?? 0))
            continue;
        if (event.trigger === "pool" && poolSize < resolveGiftMinPool(event, habitatMinPools))
            continue;
...
export function getGiftToastTitle(gift) {
    if (gift?.trigger === "professor-transfer")
        return "博士からもらった！";
    return "ポケモンをもらった！";
}
...
export function xpToNextLevel(level) {
    const cappedLevel = clampLevel(level);
    if (cappedLevel >= MAX_MONSTER_LEVEL)
        return 0;
...
export function getSpecies(level, lineId = DEFAULT_MONSTER_LINE_ID) {
    ...
    if (!line)
        throw new Error(`Monster line ${lineId} not found`);
    return ([...line.species].reverse().find(s => level >= s.minLevel) ??
        line.species[0]);
}
/**
 * 累計 XP からモンスターの状態を返す
...
export function getItemEvolutionCandidates(collection, partyOnly = true) {
    ...
    for (const monster of normalized.monsters) {
        if (!partyIds.has(monster.id))
            continue;
        if (monster.heldItemType)
            continue;
