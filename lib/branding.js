...
/** アプリのブランド名（layout.tsx のタイトル等で使用予定） */
export const BRAND = Object.freeze({
  name: "【独自IP未定】英単語クエスト", // TODO: ポケモン非依存の正式名称に確定する
  tagline: "英単語を記述式で答える学習RPG",
});
...
// IP依存の用語 → 中立/独自用語への対応表（リネームの正本）。
// コード・UI文言の置換時はこの表を参照して表記を統一する。
export const TERM_GLOSSARY = Object.freeze({
  ポケモン: "モンスター",
  マスターボール: "【独自IP】キャプチャーオーブ（上位）",
  ボール: "【独自IP】キャプチャーオーブ",
  御三家: "スターターモンスター",
  // トレーナー/ジム/博士 は一般的RPG用語に近いが、世界観の独自性確保のため
  // 必要に応じて独自呼称へ寄せる。
});
