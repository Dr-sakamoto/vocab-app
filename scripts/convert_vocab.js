...
const vocabModule = await import('data:text/javascript;base64,' + Buffer.from(content).toString('base64'));
if (!vocabModule.QUESTIONS) throw new Error('QUESTIONS not found');
const questions = vocabModule.QUESTIONS.map(({ target, answers }) => ({ target, answers }));
